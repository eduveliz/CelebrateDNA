...
module.exports = createPreview = async (nameFile, propiedades, orderInfo) => {
    ...
    const font = fontStyle(properties[18].value);
    const statement = properties[19].value;
    const personalStatementOne = statement === "Replicate the map on both sides" ? "" : properties[20].value;
    const personalStatementTwo = statement === "Replicate the map on both sides" ? "" : properties[21].value;
