...
module.exports = createPreview = async (nameFile, propiedades) => {
    ...
    const colorProduct = properties[20];
    //Headline
    const headline = properties[18] === "Personalized headline" ? properties[19] : properties[18];

    const statement = properties[22];
    const personalStatementOne = statement === "Replicate the map on both sides" ? "" : properties[23];
    const personalStatementTwo = statement === "Replicate the map on both sides" ? "The image to the left will be duplicated on both sides of tote." : properties[24];
    const personalStatementThree = statement === "Replicate the map on both sides" ? "" : properties[25];

    //FontSize
    const font = fontStyle(properties[21]);
