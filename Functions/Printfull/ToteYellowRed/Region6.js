...
module.exports = createPreview = async (nameFile, propiedades) => {
    ...
    const backgroundColor = colorBackground(properties[13].value);
    const backgroundLineWorld = backgroundColor === "transparent" ? "black" : "none";
    const colorProduct = properties[16].value;
    //Headline
    const headline = properties[14].value === "Personalized headline" ? properties[15].value : properties[14].value;

    const statement = properties[18].value;
    const personalStatementOne = statement === "Replicate the map on both sides" ? "" : properties[19].value;
    const personalStatementTwo = statement === "Replicate the map on both sides" ? "The image to the left will be duplicated on both sides of tote." : properties[20].value;
    const personalStatementThree = statement === "Replicate the map on both sides" ? "" : properties[21].value;

    //FontSize
    const font = fontStyle(properties[17].value);
