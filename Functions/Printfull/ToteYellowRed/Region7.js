...
module.exports = createPreview = async (nameFile, propiedades) => {
    ...
    const colorProduct = properties[18].value;
    //Headline
    const headline = properties[16].value === "Personalized headline" ? properties[17].value : properties[16].value;

    const statement = properties[20].value;
    const personalStatementOne = statement === "Replicate the map on both sides" ? "" : properties[21].value;
    const personalStatementTwo = statement === "Replicate the map on both sides" ? "The image to the left will be duplicated on both sides of tote." : properties[22].value;
    const personalStatementThree = statement === "Replicate the map on both sides" ? "" : properties[23].value;
    //FontSize
    const font = fontStyle(properties[19].value);
